...
The RNA-seq analysis has not been run yet. The **phenotype analysis** (Methods + Results — physiology, morphology, growth, source-thicket variation, thermal context) is finished and reproduces from one command; the narrative (Introduction, Discussion, Abstract) is out of scope here. Treat this file as background for the analyst, not a prescribed pipeline.
...
| Per-source resilience scores | `output/tables/19_genet_resilience_summary.csv` |
...
**144 libraries = 4 tanks × 3 source thickets × 2 wound states × 3 days × 2 temperatures.**
...
- **Source-thicket label:** 3 field-collected parent thickets A, C, D (field labels previously used as genet proxies; not yet matched to Cunning's numbered genets).
...
- **Counts:** per temp × day × wound = 4 tanks × 3 source thickets = 12; per temp × day (W+U) = 24; per day (both temps) = 48; × 3 days = **144 libraries**.
...
## 3. Analysis proposal & source/genotype matching plan

The analyst chooses the expression tool and normalization, but the minimum design constraints are fixed: include temperature, wound state, day, source-thicket label, tank/blocking, plate, extraction batch, and library-prep batch in the design or in documented QC checks. The phenotype results raise the questions below; the expression data can **test, extend, or revise** each. We name predicted *processes*, not gene symbols. Start genome-wide, then use the gene sets for interpretation.
...
2. **What distinguishes resilient source C from sensitive A, D?** C holds onto photochemistry, pigmentation, and symbionts far better (heat effect 2.7–3.5× weaker than A/D). Test for a smaller heat-induced shift and/or higher expression of stress-response genes before or early in the heat response. Because the current 144-library design is D1/D3/D10, do not call this a D0 baseline test unless D0 libraries are added. Main comparison: source C vs sources A/D by temperature, prioritizing unwounded margins where the source spread is sharpest. Then refit or sensitivity-check that contrast with final genetic PCs, kinship, or genotype clusters when available. A WGCNA module tracking `19_genet_resilience_summary.csv` is one route.
...
4. **Does wounding change source or genotype-linked differences?** The source heat-sensitivity spread remains strong in both scopes: unwounded margins (A = 0.99, D = 0.90, C = 0.43) and wounded margins/morphology (A = 0.56, D = 0.44, C = -0.05). Test whether the source effect and source-by-temperature effect differ by wound state, then ask whether that pattern is better explained by final genetic PCs/kinship/clusters. Symbiont (*Symbiodiniaceae*) reads, if retained after host/symbiont mapping, could corroborate the symbiont-density loss and C's retention.

**Final genotype matching (A/C/D-derived samples ↔ Cunning genets) — high-value if the SNP match works.** Cunning et al. 2024 (*Coral Reefs*, doi:10.1007/s00338-024-02577-7) measured **acute CBASS Fv/Fm ED50 for 20 genotyped *A. pulchra* genets from Mahana** (range 34.4–36.6 °C; ED50 predicts bleaching, R = 0.74; collected Dec 2022, "mahana"). We measured **chronic** resilience for 3 source thickets from the same site. The test: call genotype-distinguishing SNPs from the host RNA-seq reads, match A/C/D-derived samples to Cunning's reference, then ask whether **acute CBASS ED50 predicts our chronic wound-context source ranking (C > D > A)** — a cross-method validation and a link to source/genotype-by-temperature expression differences.
...
| 2 | Source C resilience (multivariate displacement 1.02 vs 3.71 A / 3.38 D); spread largest unwounded | **Source C vs A/D × temperature, unwounded margins**; WGCNA vs resilience score. Barshis 2013 frontloading |
| 3 | Wounding homogenizes source response (spread compresses when wounded) | **Source × wound** — source + source×temperature effect smaller in wounded margins |
...
| Does source C differ from A/D? | frontloaded or dampened stress modules, redox/proteostasis, symbiosis, cell adhesion, ion transport | temperature x source-thicket label, with preliminary SNP clusters/PCs used only as exploratory covariates until final SNPs arrive |
...
- **C. Source/genotype-linked thermal tolerance of regeneration (cross-method).** Regenerative capacity differs strongly among sources (C vs A/D); final SNPs can test how much of that signal is genotype/kinship-linked and whether it pairs with the independent acute CBASS assay (Cunning et al. 2024). Can sit under A or B, or stand alone as a Discussion section.
